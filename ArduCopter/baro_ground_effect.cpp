...
void Copter::update_ground_effect_detector(void)
{
    ...
    if (pos_control->NE_reference_is_active()) {
    ...
    bool xy_speed_demand_low = pos_control->NE_reference_is_active() && des_speed_ne_ms <= 1.25;
    bool slow_horizontal = xy_speed_demand_low || (xy_speed_low && !pos_control->NE_reference_is_active()) || (flightmode->mode_number() == Mode::Number::ALT_HOLD && small_angle_request);

    bool descent_demanded = pos_control->D_reference_is_active() && des_climb_rate_ms < 0.0f;
